SELECT mety.mety_name AS `meal-type`, al.al_id AS `id`, al.al_name AS `name`, meal.quantity AS `quantity`, 
    un.un_name AS `unit`, alun.grams_per_unit AS `grams-per-unit`, al.al_hydrates AS `hydrates` 
    FROM MEAL_TYPE mety
        ...
        INNER JOIN ALIMENT_UNIT alun ON alun.al_id = al.al_id
    WHERE me.us_id = 1 AND me.me_date = '2018-08-22'
UNION
SELECT mety.mety_name AS `meal-type`, re.re_id AS `id`, re.re_name AS `name`, NULL AS `quantity`, NULL AS `unit`, 
    NULL AS `grams-per-unit`, SUM(al.al_hydrates) AS `hydrates`
    FROM MEAL_TYPE mety
        INNER JOIN MEAL me ON me.mety_id = mety.mety_id
        ...
        INNER JOIN UNIT un ON un.un_id = ral.un_id
